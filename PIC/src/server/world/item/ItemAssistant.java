...
public class ItemAssistant {
        ...
        for (int i = 0; i < c.playerItems.length; i++) {
            if (o != null) {
                if (tradeable(c.playerItems[i] - 1)) {
                    ...
                    Server.itemHandler.createGroundItem(c, c.playerItems[i] - 1, c.getX(), c.getY(), c.playerItemsN[i], c.slot);
                }
            } else {
                Server.itemHandler.createGroundItem(c, c.playerItems[i] - 1, c.getX(), c.getY(), c.playerItemsN[i], c.slot);
            }
        ...
        for (int e = 0; e < c.playerEquipment.length; e++) {
            if (o != null) {
                if (tradeable(c.playerEquipment[e])) {
                    ...
                    Server.itemHandler.createGroundItem(c, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e], c.slot);
                }
            } else {
                Server.itemHandler.createGroundItem(c, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e], c.slot);
            }
    ...
    public void deleteEquipment(int i, int j) {
        // synchronized(c) {
        if (Server.playerHandler.players[c.slot] == null) {
            return;
        }
        if (i < 0) {
            return;
