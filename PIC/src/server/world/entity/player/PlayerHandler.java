                ...
                try {
                    ...
                    if (players[i].disconnected && (System.currentTimeMillis() - players[i].logoutDelay > 10000 || players[i].properLogout || kickAllPlayers)) {
                        if (players[i].inTrade) {
                            Player o = Server.playerHandler.players[players[i].tradeWith];
                            if (o != null) {
                                o.getTradeAndDuel().declineTrade();
                            }
                        }
                        if (players[i].duelStatus == 5) {
                            Player o = Server.playerHandler.players[players[i].duelingWith];
                            if (o != null) {
            ...
            for (int i = 0; i < Config.MAX_PLAYERS; i++) {
                ...
                try {
                    if (players[i].disconnected && (System.currentTimeMillis() - players[i].logoutDelay > 10000 || players[i].properLogout || kickAllPlayers)) {
                        ...
                        if (players[i].duelStatus == 5) {
                            Player o1 = Server.playerHandler.players[players[i].duelingWith];
                            if (o1 != null) {
                        ...
                        Player o1 = Server.playerHandler.players[i];
        ...
        synchronized (plr) {
            ...
            for (int i = 0; i < NpcHandler.maxNPCs; i++) {
                if (Server.npcHandler.npcs[i] != null) {
                    int id = Server.npcHandler.npcs[i].slot;
