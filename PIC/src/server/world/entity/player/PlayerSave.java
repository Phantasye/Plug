...
import server.util.io.Loadable;
import core.ui.components.Console;

public class PlayerSave implements Loadable {
			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
						...
						p.rights = PlayerRights.getRights(Integer
								.parseInt(token2));
	...
	public static boolean saveGame(Player p) {
		...
		if (p.playerName == null
				|| Server.playerHandler.players[p.slot] == null) {
		...
		try {
			Path dst = Paths.get("./data/characters/" + p.playerName + ".txt");
			OutputStream out = Files.newOutputStream(dst);
			characterfile = new BufferedWriter(new OutputStreamWriter(out));
			...
			characterfile.write(
					Integer.toString(p.rights.getRightsAsInteger()), 0, Integer
							.toString(p.rights.getRightsAsInteger()).length());
			characterfile.newLine();
