...
import server.world.content.transport.Teleport;
...
public class PlayerAssistant {
	...
	public void frame1() {
		...
		for (int i = 0; i < Config.MAX_PLAYERS; i++) {
			if (Server.playerHandler.players[i] != null) {
				Player person = Server.playerHandler.players[i];
				if (person != null) {
	...
	public void applyDead() {
		...
		if (c.duelStatus != 6) {
			...
			Player o = Server.playerHandler.players[c.killerId];
			if (o != null) {
	...
	public void resetFollowers() {
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				if (Server.playerHandler.players[j].followId == c.slot) {
	...
	public int getNpcId(int id) {
		for (int i = 0; i < NpcHandler.maxNPCs; i++) {
			if (NpcHandler.npcs[i] != null) {
				if (Server.npcHandler.npcs[i].slot == id) {
					return i;
				}
			}
		}
