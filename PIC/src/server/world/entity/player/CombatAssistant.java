...
public class CombatAssistant {
    ...
    public void attackNpc(int i) {
        if (Server.npcHandler.npcs[i] != null) {
            ...
            if (Server.npcHandler.npcs[i].underAttackBy > 0 && Server.npcHandler.npcs[i].underAttackBy != c.slot && !Server.npcHandler.npcs[i].inMulti()) {
                c.npcIndex = 0;
            ...
            if (Server.npcHandler.npcs[i].spawnedBy != c.slot && Server.npcHandler.npcs[i].spawnedBy > 0) {
                resetPlayerAttack();
            ...
            if (c.attackTimer <= 0) {
                ...
                // c.specAccuracy = 1.0;
                // c.specDamage = 1.0;
                Server.npcHandler.npcs[i].underAttackBy = c.slot;
    ...
    public void delayedHit(int i) { // npc hit delay
        if (Server.npcHandler.npcs[i] != null) {
            ...
            Server.npcHandler.npcs[i].facePlayer(c.slot);

            if (Server.npcHandler.npcs[i].underAttackBy > 0 && Server.npcHandler.getsPulled(i)) {
                Server.npcHandler.npcs[i].killerId = c.slot;
            ...
            if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
                ...
                Server.npcHandler.npcs[i].dealHit(new Hit(damage, HitType.NORMAL));
                if (damage2 > -1) {
                    Server.npcHandler.npcs[i].dealSecondaryHit(new Hit(damage2, HitType.NORMAL));
    ...
    public void applyNpcMeleeDamage(int i, int damageMask) {
        ...
        switch (damageMask) {
            ...
            case 2:
                Server.npcHandler.npcs[i].dealSecondaryHit(new Hit(damage, HitType.NORMAL));
                c.totalDamageDealt += damage;
                c.doubleHit = false;
                break;
    ...
    public void attackPlayer(int i) {
        ...
        if (Server.playerHandler.players[i] != null) {
            ...
            if (c.attackTimer <= 0) {
                ...
                if (c.duelStatus != 5) {
                    if (!c.attackedPlayers.contains(c.playerIndex) && !Server.playerHandler.players[c.playerIndex].attackedPlayers.contains(c.slot)) {
                ...
                Server.playerHandler.players[i].underAttackBy = c.slot;
                ...
                Server.playerHandler.players[i].killerId = c.slot;
                c.lastArrowUsed = 0;
                c.rangeItemUsed = 0;
                if (!usingBow && !c.usingMagic && !usingOtherRangeWeapons) { // melee
                    // hit
    ...
    public void playerDelayedHit(int i) {
        if (Server.playerHandler.players[i] != null) {
            ...
            if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
                ...
                Server.playerHandler.players[i].damageTaken[c.slot] += damage;
                c.killedBy = Server.playerHandler.players[i].slot;
                // Server.playerHandler.players[i].handleHitMask(damage);
                Server.playerHandler.players[i].dealHit(new Hit(damage));
                if (damage2 != -1) {
                    // Server.playerHandler.players[i].playerLevel[3] -=
                    // damage2;
                    Server.playerHandler.players[i].dealSecondaryHit(new Hit(damage2));
                    Server.playerHandler.players[i].damageTaken[c.slot] += damage2;
    ...
    public void appendMultiBarrage(int playerId, boolean splashed) {
        if (Server.playerHandler.players[playerId] != null) {
            Player c2 = Server.playerHandler.players[playerId];
            ...
            if (checkMultiBarrageReqs(playerId)) {
                ...
                if (Misc.random(mageAtk()) > Misc.random(mageDef()) && !c.magicFailed) {
                    ...
                    Server.playerHandler.players[playerId].dealHit(new Hit(damage));
                    Server.playerHandler.players[playerId].damageTaken[c.slot] += damage;
    ...
    public void applyPlayerMeleeDamage(int i, int damageMask) {
        ...
        Server.playerHandler.players[i].killerId = c.slot;
        Server.playerHandler.players[i].singleCombatDelay = System.currentTimeMillis();
        if (c.killedBy != Server.playerHandler.players[i].slot)
        ...
        switch (damageMask) {
            ...
            case 2:
                 ...
                 */
                // Server.playerHandler.players[i].playerLevel[3] -= damage;
                Server.playerHandler.players[i].dealSecondaryHit(new Hit(damage));
                Server.playerHandler.players[i].damageTaken[c.slot] += damage;
                c.totalPlayerDamageDealt += damage;
                c.doubleHit = false;
                break;
    ...
    public void applySmite(int index, int damage) {
        ...
        if (Server.playerHandler.players[index] != null) {
            Player c2 = Server.playerHandler.players[index];
    ...
    public boolean checkReqs() {
        ...
        if (Config.SINGLE_AND_MULTI_ZONES) {
            if (!Server.playerHandler.players[c.playerIndex].inMulti()) { // single
                // combat
                // zones
                if (Server.playerHandler.players[c.playerIndex].underAttackBy != c.slot && Server.playerHandler.players[c.playerIndex].underAttackBy != 0) {
                    c.sendMessage("That player is already in combat.");
                    c.stopMovement();
                    c.getCombat().resetPlayerAttack();
                    return false;
                }
                if (Server.playerHandler.players[c.playerIndex].slot != c.underAttackBy && c.underAttackBy != 0 || c.underAttackBy2 > 0) {
    ...
    public boolean checkMultiBarrageReqs(int i) {
        ...
        if (Config.SINGLE_AND_MULTI_ZONES) {
            if (!Server.playerHandler.players[i].inMulti()) { // single combat
                // zones
                if (Server.playerHandler.players[i].underAttackBy != c.slot && Server.playerHandler.players[i].underAttackBy != 0) {
                    return false;
                }
                if (Server.playerHandler.players[i].slot != c.underAttackBy && c.underAttackBy != 0) {
                    c.sendMessage("You are already in combat.");
                    return false;
                }
            }
    ...
    public void handleDfs() {
        if (System.currentTimeMillis() - c.dfsDelay > 30000) {
            if (c.playerIndex > 0 && Server.playerHandler.players[c.playerIndex] != null) {
                ...
                c.gfx0(600);
                Server.playerHandler.players[c.playerIndex].dealSecondaryHit(new Hit(damage));
    ...
    public void handleDfsNPC() {
        if (System.currentTimeMillis() - c.dfsDelay > 30000) {
            if (c.npcIndex > 0 && Server.npcHandler.npcs[c.npcIndex] != null) {
                ...
                c.startAnimation(2836);
                c.gfx0(600);
                Server.npcHandler.npcs[c.npcIndex].dealSecondaryHit(new Hit(damage, HitType.NORMAL));
                c.dfsDelay = System.currentTimeMillis();
            } else {
                c.sendMessage("I should be in combat before using this.");
            }
    ...
    public void handleGmaulPlayer() {
        if (c.playerIndex > 0) {
            Player o = Server.playerHandler.players[c.playerIndex];
