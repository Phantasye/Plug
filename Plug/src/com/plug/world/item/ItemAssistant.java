...
public class ItemAssistant {
    ...
    public void dropAllItems() {
        Player o = Game.playerHandler.players[c.killerId];

        for (int i = 0; i < c.playerItems.length; i++) {
            if (o != null) {
                if (tradeable(c.playerItems[i] - 1)) {
                    Game.itemHandler.createGroundItem(o, c.playerItems[i] - 1, c.getX(), c.getY(), c.playerItemsN[i], c.killerId);
                } else {
                    if (specialCase(c.playerItems[i] - 1))
                        Game.itemHandler.createGroundItem(o, 995, c.getX(), c.getY(), getUntradePrice(c.playerItems[i] - 1), c.killerId);
                    Game.itemHandler.createGroundItem(c, c.playerItems[i] - 1, c.getX(), c.getY(), c.playerItemsN[i], c.slot);
                }
            } else {
                Game.itemHandler.createGroundItem(c, c.playerItems[i] - 1, c.getX(), c.getY(), c.playerItemsN[i], c.slot);
            }
        ...
        for (int e = 0; e < c.playerEquipment.length; e++) {
            if (o != null) {
                if (tradeable(c.playerEquipment[e])) {
                    Game.itemHandler.createGroundItem(o, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e], c.killerId);
                } else {
                    if (specialCase(c.playerEquipment[e]))
                        Game.itemHandler.createGroundItem(o, 995, c.getX(), c.getY(), getUntradePrice(c.playerEquipment[e]), c.killerId);
                    Game.itemHandler.createGroundItem(c, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e], c.slot);
                }
            } else {
                Game.itemHandler.createGroundItem(c, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e], c.slot);
            }
        ...
        if (o != null) {
            Game.itemHandler.createGroundItem(o, 526, c.getX(), c.getY(), 1, c.killerId);
        }
    }
        ...
        if ((((freeSlots() >= 1) || playerHasItem(item, 1)) && Item.itemStackable[item]) || ((freeSlots() > 0) && !Item.itemStackable[item])) {
            for (int i = 0; i < c.playerItems.length; i++) {
                if ((c.playerItems[i] == (item + 1)) && Item.itemStackable[item] && (c.playerItems[i] > 0)) {
                    ...
                    if (((c.playerItemsN[i] + amount) < Constants.MAXITEM_AMOUNT) && ((c.playerItemsN[i] + amount) > -1)) {
        ...
        for (int i = 0; i < c.playerEquipment.length; i++) {
            if (c.playerEquipment[i] > -1) {
                for (int j = 0; j < Constants.ITEM_LIMIT; j++) {
                    if (Game.itemHandler.ItemList[j] != null) {
                        if (Game.itemHandler.ItemList[j].itemId == c.playerEquipment[i]) {
                            for (int k = 0; k < c.playerBonus.length; k++) {
                                c.playerBonus[k] += Game.itemHandler.ItemList[j].Bonuses[k];
                            }
                            break;
                        }
                    }
    ...
    public void deleteEquipment(int i, int j) {
        // synchronized(c) {
        if (Game.playerHandler.players[c.slot] == null) {
            return;
        }
        if (i < 0) {
            return;
    ...
    public void dropArrowNpc() {
        if (c.playerEquipment[c.playerCape] == 10499)
            return;
        int enemyX = Game.npcHandler.npcs[c.oldNpcIndex].getX();
        int enemyY = Game.npcHandler.npcs[c.oldNpcIndex].getY();
        if (Misc.random(10) >= 4) {
            if (Game.itemHandler.itemAmount(c.rangeItemUsed, enemyX, enemyY) == 0) {
                Game.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, 1, c.getId());
            } else if (Game.itemHandler.itemAmount(c.rangeItemUsed, enemyX, enemyY) != 0) {
                int amount = Game.itemHandler.itemAmount(c.rangeItemUsed, enemyX, enemyY);
                Game.itemHandler.removeGroundItem(c, c.rangeItemUsed, enemyX, enemyY, false);
                Game.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, amount + 1, c.getId());
            }
        }
    }

    public void dropArrowPlayer() {
        int enemyX = Game.playerHandler.players[c.oldPlayerIndex].getX();
        int enemyY = Game.playerHandler.players[c.oldPlayerIndex].getY();
        if (c.playerEquipment[c.playerCape] == 10499)
            return;
        if (Misc.random(10) >= 4) {
            if (Game.itemHandler.itemAmount(c.rangeItemUsed, enemyX, enemyY) == 0) {
    ...
    public String getItemName(int ItemID) {
        for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
            if (Game.itemHandler.ItemList[i] != null) {
                if (Game.itemHandler.ItemList[i].itemId == ItemID) {
                    return Game.itemHandler.ItemList[i].itemName;
                }
            }
        }
        return "Unarmed";
    ...
    public int getItemId(String itemName) {
        for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
            if (Game.itemHandler.ItemList[i] != null) {
                if (Game.itemHandler.ItemList[i].itemName.equalsIgnoreCase(itemName)) {
                    return Game.itemHandler.ItemList[i].itemId;
                }
            }
        }
        return -1;
    ...
    public int getUnnotedItem(int ItemID) {
        ...
        for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
            if (Game.itemHandler.ItemList[i] != null) {
                if (Game.itemHandler.ItemList[i].itemId == ItemID) {
                    NotedName = Game.itemHandler.ItemList[i].itemName;
                }
            }
        }
        for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
            if (Game.itemHandler.ItemList[i] != null) {
                if (Game.itemHandler.ItemList[i].itemName == NotedName) {
                    if (Game.itemHandler.ItemList[i].itemDescription.startsWith("Swap this note at any bank for a") == false) {
                        NewID = Game.itemHandler.ItemList[i].itemId;
                        break;
                    }
                }
            }
