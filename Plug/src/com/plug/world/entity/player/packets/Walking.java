...
public class Walking implements PacketType {
    ...
    public void processPacket(Player c, int packetType, int packetSize) {
        ...
        if (c.duelRule[1] && c.duelStatus == 5) {
            if (Game.playerHandler.players[c.duelingWith] != null) {
                if (!c.goodDistance(c.getX(), c.getY(), Game.playerHandler.players[c.duelingWith].getX(), Game.playerHandler.players[c.duelingWith].getY(), 1) || c.attackTimer == 0) {
        ...
        if (c.freezeTimer > 0) {
            if (Game.playerHandler.players[c.playerIndex] != null) {
                if (c.goodDistance(c.getX(), c.getY(), Game.playerHandler.players[c.playerIndex].getX(), Game.playerHandler.players[c.playerIndex].getY(), 1) && packetType != 98) {
                    c.playerIndex = 0;
                    return;
                }
            }
