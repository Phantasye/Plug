...
public class TradeAndDuel {
    ...
    public void requestTrade(int id) {
        try {
            Player o = Game.playerHandler.players[id];
            if (id == c.slot)
                return;
            c.tradeWith = id;
            if (!c.inTrade && o.tradeRequested && o.tradeWith == c.slot) {
    ...
    public void openTrade() {
        Player o = Game.playerHandler.players[c.tradeWith];
    ...
    public void resetOTItems(int WriteFrame) {
        synchronized (c) {
            ...
            for (GameItem item : o.getTradeAndDuel().offeredItems) {
                if (item.amount > 254) {
                    c.getOutStream().writeByte(255); // item's stack count.
                    // if
                    // over 254, write byte
                    // 255
                    c.getOutStream().writeDWord_v2(item.amount);
    ...
    public void startDuel() {
        ...
        c.getPA().createPlayerHints(10, o.slot);
            ...
            if (item.id > 0 && item.amount > 0) {
                if (Item.itemStackable[item.id]) {
                    if (!c.getItems().addItem(item.id, item.amount)) {
                        Game.itemHandler.createGroundItem(c, item.id, c.getX(), c.getY(), item.amount, c.getId());
                    }
                } else {
                    int amount = item.amount;
                    for (int a = 1; a <= amount; a++) {
                        if (!c.getItems().addItem(item.id, 1)) {
                            Game.itemHandler.createGroundItem(c, item.id, c.getX(), c.getY(), 1, c.getId());
                        }
                    }
                }
            }
