...
public class PlayerHandler {
                ...
                try {
                    ...
                    if (players[i].disconnected && (System.currentTimeMillis() - players[i].logoutDelay > 10000 || players[i].properLogout || kickAllPlayers)) {
                        if (players[i].inTrade) {
                            Player o = Game.playerHandler.players[players[i].tradeWith];
                            if (o != null) {
                                o.getTradeAndDuel().declineTrade();
                            }
                        }
                        if (players[i].duelStatus == 5) {
                            Player o = Game.playerHandler.players[players[i].duelingWith];
                            if (o != null) {
            ...
            for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
                ...
                try {
                    if (players[i].disconnected && (System.currentTimeMillis() - players[i].logoutDelay > 10000 || players[i].properLogout || kickAllPlayers)) {
                        ...
                        if (players[i].duelStatus == 5) {
                            Player o1 = Game.playerHandler.players[players[i].duelingWith];
                            if (o1 != null) {
                        ...
                        Player o1 = Game.playerHandler.players[i];
        ...
        synchronized (plr) {
            ...
            for (int i = 0; i < NpcHandler.maxNPCs; i++) {
                if (Game.npcHandler.npcs[i] != null) {
                    int id = Game.npcHandler.npcs[i].slot;
                    if (plr.RebuildNPCList == false && (plr.npcInListBitmap[id >> 3] & (1 << (id & 7))) != 0) {

                    } else if (plr.withinDistance(Game.npcHandler.npcs[i]) == false) {

                    } else {
                        plr.addNewNPC(Game.npcHandler.npcs[i], str, updateBlock);
                    }
                }
            }
    ...
    public void updatePlayer(Player plr, Stream str) {
        ...
        int j = 0;
        for (int i = 0; i < Constants.MAX_PLAYERS; i++) { // memory leak fix
