...
import com.plug.world.content.transport.Teleport;
...
public class PlayerAssistant {
	...
	public PlayerAssistant(Player c) {
		this.c = c;
	}
	...
	public void teleport(Teleport teleport) {
		...
		try {
			teleport.execute(c);
		} catch (NoRunesException | InsufficientRightsException
		...
		for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
			if (PlayerHandler.players[i] != null) {
				if (PlayerHandler.players[i].playerName
					...
					Player player = (Player) PlayerHandler.players[i];
					return player;
				}
			}
		}
	...
	public void setPrivateMessaging(int i) { // friends and ignore list
		// status
		// synchronized(c) {
		if (c.getOutStream() != null && c != null) {
	...
	public void frame1() {
		...
		for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
			if (Game.playerHandler.players[i] != null) {
				Player person = Game.playerHandler.players[i];
				if (person != null) {
			...
			if (person != null) {
				if (person.getOutStream() != null) {
					if (person.distanceToPoint(x, y) <= 25) {
						if (person.heightLevel == c.heightLevel)
	...
	public void createPlayersStillGfx(int id, int x, int y, int height, int time) {
		// synchronized(c) {
		for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
			Player p = Game.playerHandler.players[i];
			if (p != null) {
				Player person = p;
				if (person != null) {
					if (person.getOutStream() != null) {
	...
	public void logIntoPM() {
		...
		for (int i1 = 0; i1 < Constants.MAX_PLAYERS; i1++) {
			Player p = Game.playerHandler.players[i1];
		...
		for (int i = 0; i < c.friends.length; i++) {
			if (c.friends[i] != 0) {
				for (int i2 = 1; i2 < Constants.MAX_PLAYERS; i2++) {
					Player p = Game.playerHandler.players[i2];
					if (p != null
							&& p.isActive
	...
	public void updatePM(int pID, int world) { // used for private chat updates
		Player p = Game.playerHandler.players[pID];
		...
		long l = Misc
				.playerNameToInt64(Game.playerHandler.players[pID].playerName);
	...
	public void resetDamageDone() {
		for (int i = 0; i < PlayerHandler.players.length; i++) {
			if (PlayerHandler.players[i] != null) {
				PlayerHandler.players[i].damageTaken[c.slot] = 0;
			}
		}
	}
	...
	public void resetFollowers() {
		for (int j = 0; j < Game.playerHandler.players.length; j++) {
			if (Game.playerHandler.players[j] != null) {
				if (Game.playerHandler.players[j].followId == c.slot) {
			...
			if (c.pitsStatus == 1) {
				...
				movePlayer(
						...
						Constants.DUELING_RESPAWN_Y
								+ (Misc.random(Constants.RANDOM_DUELING_RESPAWN)),
						0);
				if (c.duelStatus != 6) { // if we have won but have died,
					// don't
	...
	public void followPlayer() {
		if (Game.playerHandler.players[c.followId] == null
				|| Game.playerHandler.players[c.followId].isDead) {
			c.followId = 0;
			return;
		}
		if (c.freezeTimer > 0) {
		...
		int otherX = Game.playerHandler.players[c.followId].getX();
		int otherY = Game.playerHandler.players[c.followId].getY();
	...
	public void followNpc() {
		if (Game.npcHandler.npcs[c.followId2] == null
				|| Game.npcHandler.npcs[c.followId2].isDead) {
			c.followId2 = 0;
			return;
		}
		if (c.freezeTimer > 0) {
		...
		int otherX = Game.npcHandler.npcs[c.followId2].getX();
		int otherY = Game.npcHandler.npcs[c.followId2].getY();
		boolean withinDistance = c.goodDistance(otherX, otherY, c.getX(),
				c.getY(), 2);
		boolean goodDistance = c.goodDistance(otherX, otherY, c.getX(),
				c.getY(), 1);
	...
	public int getNpcId(int id) {
		for (int i = 0; i < NpcHandler.maxNPCs; i++) {
			if (NpcHandler.npcs[i] != null) {
				if (Game.npcHandler.npcs[i].slot == id) {
					return i;
				}
			}
		}
