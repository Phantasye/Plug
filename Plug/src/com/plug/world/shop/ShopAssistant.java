...
public class ShopAssistant {
    ...
    public ShopAssistant(Player c) {
        this.c = c;
    }

    public boolean shopSellsItem(int itemID) {
        for (int i = 0; i < Game.shopHandler.ShopItems.length; i++) {
            if (itemID == (Game.shopHandler.ShopItems[c.myShopId][i] - 1)) {
                return true;
            }
        }
        return false;
    ...
    public void openShop(int ShopID) {
        ...
        c.getPA().sendFrame126(Game.shopHandler.ShopName[ShopID], 3901);
    }

    public void updatePlayerShop() {
        for (int i = 1; i < Constants.MAX_PLAYERS; i++) {
            if (Game.playerHandler.players[i] != null) {
                if (Game.playerHandler.players[i].isShopping == true && Game.playerHandler.players[i].myShopId == c.myShopId && i != c.slot) {
                    Game.playerHandler.players[i].updateShop = true;
                }
            }
        }
    }
    ...
    public void resetShop(int ShopID) {
        synchronized (c) {
            int TotalItems = 0;
            for (int i = 0; i < Game.shopHandler.MaxShopItems; i++) {
                if (Game.shopHandler.ShopItems[ShopID][i] > 0) {
                    TotalItems++;
                }
            }
            if (TotalItems > Game.shopHandler.MaxShopItems) {
                TotalItems = Game.shopHandler.MaxShopItems;
            }
            ...
            int TotalCount = 0;
            for (int i = 0; i < Game.shopHandler.ShopItems.length; i++) {
                if (Game.shopHandler.ShopItems[ShopID][i] > 0 || i <= Game.shopHandler.ShopItemsStandard[ShopID]) {
                    if (Game.shopHandler.ShopItemsN[ShopID][i] > 254) {
                        c.getOutStream().writeByte(255);
                        c.getOutStream().writeDWord_v2(Game.shopHandler.ShopItemsN[ShopID][i]);
                    } else {
                        c.getOutStream().writeByte(Game.shopHandler.ShopItemsN[ShopID][i]);
                    }
                    if (Game.shopHandler.ShopItems[ShopID][i] > Constants.ITEM_LIMIT || Game.shopHandler.ShopItems[ShopID][i] < 0) {
                        Game.shopHandler.ShopItems[ShopID][i] = Constants.ITEM_LIMIT;
                    }
                    c.getOutStream().writeWordBigEndianA(Game.shopHandler.ShopItems[ShopID][i]);
                    TotalCount++;
                }
    ...
    public double getItemShopValue(int ItemID, int Type, int fromSlot) {
        ...
        for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
            if (Game.itemHandler.ItemList[i] != null) {
                if (Game.itemHandler.ItemList[i].itemId == ItemID) {
                    ShopValue = Game.itemHandler.ItemList[i].ShopValue;
                }
            }
        }
        ...
        if (Game.shopHandler.ShopBModifier[c.myShopId] == 1) {
            TotPrice *= 1;
            TotPrice *= 1;
            if (Type == 1) {
                TotPrice *= 1;
    ...
    public int getItemShopValue(int itemId) {
        for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
            if (Game.itemHandler.ItemList[i] != null) {
                if (Game.itemHandler.ItemList[i].itemId == itemId) {
                    return (int) Game.itemHandler.ItemList[i].ShopValue;
                }
            }
        }
        return 0;
    ...
    public void sellToShopPrice(int removeId, int removeSlot) {
        ...
        boolean IsIn = false;
        if (Game.shopHandler.ShopSModifier[c.myShopId] > 1) {
            for (int j = 0; j <= Game.shopHandler.ShopItemsStandard[c.myShopId]; j++) {
                if (removeId == (Game.shopHandler.ShopItems[c.myShopId][j] - 1)) {
                    IsIn = true;
                    break;
                }
            }
    ...
    public boolean sellItem(int itemID, int fromSlot, int amount) {
        ...
        if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
            if (Game.shopHandler.ShopSModifier[c.myShopId] > 1) {
                boolean IsIn = false;
                for (int i = 0; i <= Game.shopHandler.ShopItemsStandard[c.myShopId]; i++) {
                    if (itemID == (Game.shopHandler.ShopItems[c.myShopId][i] - 1)) {
                        IsIn = true;
                        break;
                    }
    ...
    public boolean addShopItem(int itemID, int amount) {
        ...
        for (int i = 0; i < Game.shopHandler.ShopItems.length; i++) {
            if ((Game.shopHandler.ShopItems[c.myShopId][i] - 1) == itemID) {
                Game.shopHandler.ShopItemsN[c.myShopId][i] += amount;
                Added = true;
            }
        }
        if (Added == false) {
            for (int i = 0; i < Game.shopHandler.ShopItems.length; i++) {
                if (Game.shopHandler.ShopItems[c.myShopId][i] == 0) {
                    Game.shopHandler.ShopItems[c.myShopId][i] = (itemID + 1);
                    Game.shopHandler.ShopItemsN[c.myShopId][i] = amount;
                    Game.shopHandler.ShopItemsDelay[c.myShopId][i] = 0;
                    break;
                }
            }
        }
    ...
    public boolean buyItem(int itemID, int fromSlot, int amount) {
        ...
        if (amount > 0) {
            if (amount > Game.shopHandler.ShopItemsN[c.myShopId][fromSlot]) {
                amount = Game.shopHandler.ShopItemsN[c.myShopId][fromSlot];
            }
            // double ShopValue;
            // double TotPrice;
            int TotPrice2 = 0;
                ...
                if (c.myShopId != 29 || c.myShopId != 30 || c.myShopId != 31) {
                    if (c.playerItemsN[Slot] >= TotPrice2) {
                        if (c.getItems().freeSlots() > 0) {
                            ...
                            Game.shopHandler.ShopItemsN[c.myShopId][fromSlot] -= 1;
                            Game.shopHandler.ShopItemsDelay[c.myShopId][fromSlot] = 0;
                            if ((fromSlot + 1) > Game.shopHandler.ShopItemsStandard[c.myShopId]) {
                                Game.shopHandler.ShopItems[c.myShopId][fromSlot] = 0;
                            }
                        } else {
                            c.sendMessage("You don't have enough space in your inventory.");
                            break;
